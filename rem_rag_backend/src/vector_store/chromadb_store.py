...
class REMVectorStore:
    ...
    def add_with_embeddings(self, texts: List[str], embeddings: List[List[float]], metadata: List[dict]) -> List[str]:
        """
        Add documents to the vector store with pre-computed embeddings.
        
        Args:
            texts: List of text documents to store
            embeddings: Pre-computed embeddings for each text
            metadata: List of metadata dicts for each document
                
        Returns:
            List of generated document IDs
        """
        if len(texts) != len(metadata) or len(texts) != len(embeddings):
        ...
        logger.info(f"Added {len(texts)} documents with pre-computed embeddings to collection")
        return ids
