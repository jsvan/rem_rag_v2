...
class REMCycle:
    ...
    def run_cycle(self, current_year: Optional[int] = None) -> List[str]:
        ...
        print(f"\n🌙 [{datetime.now().strftime('%H:%M:%S')}] Starting Batch REM cycle")
        ...
        print(f"✅ [{datetime.now().strftime('%H:%M:%S')}] Prepared {len(batch_items)} dreams for batch processing")
        
        # Step 2: Create batch file
        batch_file_path = self._create_batch_file(batch_items)
        
        # Step 3: Submit batch job
        print(f"📤 [{datetime.now().strftime('%H:%M:%S')}] Submitting batch to OpenAI...")
        ...
        print(f"⏳ [{datetime.now().strftime('%H:%M:%S')}] Waiting for batch to complete (checking every 60 seconds)...")
        ...
        print(f"📥 [{datetime.now().strftime('%H:%M:%S')}] Processing {len(results)} completed dreams...")
        rem_node_ids = self._process_batch_results(results, batch_items)
        
        print(f"✨ [{datetime.now().strftime('%H:%M:%S')}] Batch REM cycle complete! Created {len(rem_node_ids)} dream nodes.")
        return rem_node_ids
    ...
    def _prepare_batch_items(self, num_dreams: int, current_year: Optional[int]) -> List[REMBatchItem]:
        ...
        with tqdm(total=num_dreams, desc="Preparing REM dreams") as pbar:
            for i in range(num_dreams):
                try:
                    samples = self._sample_nodes(current_year)
                    if samples and len(samples) >= 3:
                        batch_items.append(REMBatchItem(
                            custom_id=f"fa_rem_rag_dream_{i}_{datetime.now().timestamp()}",
                            samples=samples,
    ...
    def _create_synthesis_request(self, item: REMBatchItem) -> dict:
        ...
        Example: If asked about state stability, start with "Stability of states facing pressures..." NOT "The implicit question explores..."
        ...
        try:
            ...
            batch_response = openai.batches.create(
                ...
                metadata={
                    ...
                    "dreams_count": str(len(batch_items)),
                    "year": str(current_year) if current_year else "unknown"
                }
            )
    ...
    def _wait_for_batch(self, batch_id: str, check_interval: int = 60, max_retries: int = 5) -> Optional[List[dict]]:
            ...
            try:
                ...
                if check_count % 5 == 1 or (elapsed_time - last_status_print) > 600:
                    ...
                    if counts and hasattr(counts, 'total') and counts.total > 0:
                        ...
                        print(f"   [{datetime.now().strftime('%H:%M:%S')}] Status: {batch.status} - {completed}/{total} ({progress:.0f}%) - checking every {check_interval}s")
                    else:
                        print(f"   [{datetime.now().strftime('%H:%M:%S')}] Status: {batch.status} - checking every {check_interval}s")
                    last_status_print = elapsed_time
                
                if batch.status.lower() == "completed":
                    # Download results
                    elapsed_minutes = (time.time() - start_time) / 60
                    print(f"✅ [{datetime.now().strftime('%H:%M:%S')}] Batch completed in {elapsed_minutes:.1f} minutes! Downloading results...")
                    ...
                    for attempt in range(max_retries):
                        try:
                            ...
                            if attempt < max_retries - 1:
                                ...
                                print(f"   ⚠️  Error downloading results, retrying in {wait_time}s...")
                    ...
                    # Still processing - sleep with appropriate interval
                ...
                if is_retryable and consecutive_errors < max_retries:
                    ...
                    print(f"   ⚠️  Server error (attempt {consecutive_errors}/{max_retries}), retrying in {wait_time}s...")
                    time.sleep(wait_time)
                else:
                    print(f"   ❌ Failed after {consecutive_errors} attempts: {e}")
                    return None
    ...
    async def run_cycle_async(self, current_year: Optional[int] = None) -> List[str]:
        """
        Run a complete REM cycle using async processing instead of batch API.
        ...
        print(f"\n🌙 [{datetime.now().strftime('%H:%M:%S')}] Starting Async REM cycle")
        print(f"📊 Database has {non_rem_count} non-REM nodes")
        print(f"🎯 Processing {num_dreams} REM dreams asynchronously...")
        
        # OPTIMIZATION: Get all nodes once instead of querying for each dream
        print(f"📥 [{datetime.now().strftime('%H:%M:%S')}] Loading node pool for efficient sampling...")
        node_pool = self._load_node_pool_efficiently(current_year)
        
        if not node_pool or len(node_pool) < 3:
            print("⚠️  Not enough nodes for REM cycle")
            return []
        
        # Sample all node triplets upfront using the cached pool
        print(f"🎲 [{datetime.now().strftime('%H:%M:%S')}] Sampling {num_dreams} node triplets...")
        batch_items = self._prepare_batch_items_fast(num_dreams, current_year, node_pool)
        
        if not batch_items:
            print("⚠️  No valid samples found for REM cycle")
            return []
        
        print(f"✅ [{datetime.now().strftime('%H:%M:%S')}] Prepared {len(batch_items)} dreams for async processing")
        
        # Process dreams asynchronously
        print(f"⚡ [{datetime.now().strftime('%H:%M:%S')}] Processing dreams asynchronously...")
        rem_node_ids = await self._process_dreams_async(batch_items)
        
        print(f"✨ [{datetime.now().strftime('%H:%M:%S')}] Async REM cycle complete! Created {len(rem_node_ids)} dream nodes.")
        return rem_node_ids
    
    async def _process_dreams_async(self, batch_items: List[REMBatchItem]) -> List[str]:
        """Process all REM dreams asynchronously."""
        rem_node_ids = []
        ...
        from tqdm import tqdm
        with tqdm(total=len(batch_items), desc="Processing REM dreams") as pbar:
            for i in range(0, len(batch_items), batch_size):
                batch = batch_items[i:i + batch_size]
                batch_tasks = []
                
                for item in batch:
                    task = self._process_single_dream_async(item)
    ...
    async def _process_single_dream_async(self, item: REMBatchItem) -> Optional[str]:
        """Process a single REM dream asynchronously."""
        try:
            ...
            question_prompt = f"{REM_QUESTION_PROMPT}\n\n{passages}"
            question = await self.llm.generate(question_prompt, temperature=0.7, max_tokens=500)
            ...
            synthesis_prompt = f"""Given the following source materials and implicit question: "{question}"
...
Synthesis:"""
            ...
            synthesis = await self.llm.generate(synthesis_prompt, temperature=0.8, max_tokens=400)
            
            # Store REM node
            node_id = self._store_rem_node(synthesis, question, item.samples, item.current_year)
            return node_id
            
        except Exception as e:
            logger.error(f"Error processing dream {item.custom_id}: {e}")
            return None
    
    def _load_node_pool_efficiently(self, current_year: Optional[int] = None) -> List[Dict]:
        """Load all non-REM nodes once for efficient sampling."""
        # Get all non-REM nodes in one query
        results = self.store.collection.get(
            where={"node_type": {"$ne": "rem"}},
            limit=50000,  # Reasonable upper limit
        ...
        for i in range(len(results["ids"])):
            nodes.append({
                ...
                "year": results["metadatas"][i].get("year") if i < len(results["metadatas"]) else None
            })
        
        return nodes
    
    def _prepare_batch_items_fast(self, num_dreams: int, current_year: Optional[int], node_pool: List[Dict]) -> List[REMBatchItem]:
        """Prepare batch items efficiently using pre-loaded node pool."""
        from tqdm import tqdm
        batch_items = []
        
        # Separate nodes by year for efficient sampling
        current_year_nodes = []
        other_nodes = []
        
        for node in node_pool:
            if current_year and node.get("year") == current_year:
                current_year_nodes.append(node)
            else:
        ...
        with tqdm(total=num_dreams, desc="Preparing REM dreams") as pbar:
            for i in range(num_dreams):
                try:
                    ...
                    # Sample 1 from current year if available
                    if current_year_nodes:
                        samples.append(random.choice(current_year_nodes))
                    
                    # Sample remaining from other nodes
                    remaining_needed = 3 - len(samples)
                    if len(other_nodes) >= remaining_needed:
                        # Sample without replacement for this dream
                        selected = random.sample(other_nodes, remaining_needed)
                        samples.extend(selected)
                    elif len(node_pool) >= 3:
                        # Fall back to sampling from entire pool
                        selected = random.sample(node_pool, 3)
                        samples = selected[:3]
                    else:
                        continue  # Skip this dream if not enough nodes
                    
                    # Convert to REMSample format
                    rem_samples = []
                    for node in samples:
                        rem_samples.append(REMSample(
                            node_id=node["id"],
                    ...
                    if len(rem_samples) >= 3:
                        batch_items.append(REMBatchItem(
                            custom_id=f"fa_rem_rag_dream_{i}_{datetime.now().timestamp()}",
                            samples=rem_samples[:3],
                            current_year=current_year
                        ))
                except Exception as e:
                    logger.error(f"Error preparing dream {i}: {e}")
